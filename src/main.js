...
function buildOverlayMain() {
  ...
  window.buildColorFilterList = function buildColorFilterList() {
    ...
    // ==================== SUMMARY (PROGRESS) ====================
    // Done = pixels that match the template color.
    // Wrong = painted pixels that do NOT match the template color.
    try {
      const gp = (typeof window !== 'undefined') ? window.bmGlobalProgress : null;
      const totalPixels = Number(t.pixelCount || 0);
      const requiredPixels = Number((typeof t.requiredPixelCount === 'number')
        ? t.requiredPixelCount
        : Object.values(t.colorPalette).reduce((sum, v) => sum + Number(v?.count || 0), 0));

      const summary = document.createElement('div');
      summary.style.margin = '4px 0 8px 0';

      if (gp && typeof gp.total === 'number') {
        const done = Number(gp.done || 0);
        const total = Number(gp.total || 0);
        const wrong = Number(gp.wrong || 0);
        const pct = total > 0 ? (Math.round((done / total) * 1000) / 10).toFixed(1) : '0.0';
        summary.innerHTML = `
          <small>
            Done: <b>${done.toLocaleString()}</b> / <b>${total.toLocaleString()}</b> (${pct}%)
            • Wrong: <b>${wrong.toLocaleString()}</b>
            • Template pixels: <b>${requiredPixels.toLocaleString()}</b>
            • Total image: <b>${totalPixels.toLocaleString()}</b>
          </small>
          <hr style="opacity:.35;">
        `;
      } else {
        // Fallback: template composition breakdown
        const known = Object.entries(t.colorPalette)
          .filter(([key]) => key !== 'other' && key !== '#deface' && key !== '222,250,206')
          .reduce((sum, [, v]) => sum + Number(v?.count || 0), 0);
        const other = Number(t.colorPalette?.other?.count || 0);
        const transparent = Number((t.colorPalette?.['222,250,206']?.count ?? t.colorPalette?.['#deface']?.count ?? t.defacePixelCount ?? 0));
        summary.innerHTML = `
          <small>
            Template pixels: <b>${requiredPixels.toLocaleString()}</b>
            • Known: <b>${known.toLocaleString()}</b>
            • Other: <b>${other.toLocaleString()}</b>
            • Transparent: <b>${transparent.toLocaleString()}</b>
            • Total image: <b>${totalPixels.toLocaleString()}</b>
          </small>
          <hr style="opacity:.35;">
        `;
    ...
    for (const [rgb, meta] of entries) {
      ...
      const progress = (typeof window !== 'undefined') ? (window.bmColorProgress || {}) : {};
      const doneForColor = Number(progress?.[rgb]?.done || 0);
      let labelText = `${doneForColor.toLocaleString()} / ${meta.count.toLocaleString()}`;

      // Special handling for "other" and "transparent"
      if (rgb === 'other') {
        swatch.style.background = '#888'; // Neutral color for "Other"
        labelText = `Other • ${labelText}`;
      } else if (rgb === '#deface' || rgb === '222,250,206') {
        swatch.style.background = '#deface';
        // Transparent pixels are NOT required
        labelText = `Transparent • ${meta.count.toLocaleString()}`;
      } else {
  ...
  window.addEventListener('message', (event) => {
    if (event?.data?.bmEvent === 'bm-rebuild-color-list' || event?.data?.bmEvent === 'bm-refresh-progress') {
