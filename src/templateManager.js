...
export default class TemplateManager {
  ...
  constructor(name, version, overlay) {
    ...
    this._lastColorListUpdate = 0; // Throttle UI refresh for progress
    this.colorProgress = new Map(); // Aggregated progress by expected color key
  }
  ...
  async createTemplate(blob, name, coords) {
    ...
    // "Colored" = all non-transparent pixels in the source image (includes the special Transparent marker and "other")
    const coloredCountFormatted = new Intl.NumberFormat().format(template.requiredPixelCount ?? 0);
    this.overlay.handleDisplayStatus(
      `Template created at ${coords.join(', ')}! Total pixels: ${pixelCountFormatted} • Colored pixels: ${coloredCountFormatted}`
    );
  ...
  async drawTemplateOnTile(tileBlob, tileCoords) {
    ...
    // Track progress by expected color (per tile)
    let byColor = {}; // per-tile progress by expected template color
      ...
      if (tilePixels) {
        try {
          ...
          const bump = (key, field) => {
            if (!key) { return; }
            if (!byColor[key]) { byColor[key] = { done: 0, required: 0, wrong: 0 }; }
            byColor[key][field] = (byColor[key][field] || 0) + 1;
          };
          ...
          for (let y = 0; y < tempHeight; y++) {
            for (let x = 0; x < tempWidth; x++) {
              ...
              const tplRgb = `${templatePixelCenterRed},${templatePixelCenterGreen},${templatePixelCenterBlue}`;
              const tplKey = (activeTemplate?.allowedColorsSet && activeTemplate.allowedColorsSet.has(tplRgb)) ? tplRgb : 'other';

              requiredCount++;
              bump(tplKey, 'required');
    ...
    if (templateCount > 0) {
      ...
      // Aggregate across tiles we've processed
      let aggPainted = 0;
      ...
      for (const stats of this.tileProgress.values()) {
        ...
        const bc = stats.byColor || {};
        for (const [k, v] of Object.entries(bc)) {
          if (!aggByColor[k]) { aggByColor[k] = { done: 0, required: 0, wrong: 0 }; }
          aggByColor[k].done += Number(v?.done || 0);
          aggByColor[k].required += Number(v?.required || 0);
          aggByColor[k].wrong += Number(v?.wrong || 0);
        }
      }
      ...
      // Update globals for UI (best-effort)
      try {
        window.bmGlobalProgress = { done: aggPainted, total: totalRequired, wrong: aggWrong };
        window.bmColorProgress = aggByColor;
      } catch (_) {}

      // Throttled UI refresh for the color list (if open)
      try {
        const now = Date.now();
        if (now - (this._lastColorListUpdate || 0) > 1000) {
          this._lastColorListUpdate = now;
          window.postMessage({ source: 'blue-marble', bmEvent: 'bm-refresh-progress' }, '*');
        }
      } catch (_) {}

      const doneStr = new Intl.NumberFormat().format(aggPainted);
      const totalStr = new Intl.NumberFormat().format(totalRequired);
      const wrongStr = new Intl.NumberFormat().format(aggWrong);
      const pct = totalRequired > 0 ? (Math.round((aggPainted / totalRequired) * 1000) / 10).toFixed(1) : '0.0';

      this.overlay.handleDisplayStatus(
        `Displaying ${templateCount} template${templateCount == 1 ? '' : 's'}.
Done ${doneStr} / ${totalStr} (${pct}%) • Wrong ${wrongStr}`
      );
    } else {
